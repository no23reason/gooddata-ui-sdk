...
import { DashboardRecording, DashboardsDefinition } from "./dashboards";

function createRecording(directory: string, dashboard: string): IRecording | null {
    try {
...
function loadRecordings(recordingDefinition: string): IRecording[] {
    ...
    return Object.entries(requestedDisplayForms)
        .map(([dashboard]) => createRecording(directory, dashboard))
